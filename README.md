...
APK 位于 `app/build/outputs/apk/debug/app-debug.apk`。安装应用后首次启动会请求 Root 权限；允许后，应用自动通过 Magisk 安装内置模块，并在完成后提示重启设备。模块已安装且版本一致时会直接进入服务列表。

自动安装要求设备使用 Magisk。若模块曾在 Magisk 中被手动停用，应用会保留停用状态，不会擅自重新启用。上面的模块 ZIP 仍可用于手动安装和故障恢复。
